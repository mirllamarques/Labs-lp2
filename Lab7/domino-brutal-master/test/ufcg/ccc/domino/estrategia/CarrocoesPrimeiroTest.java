...
class CarrocoesPrimeiroTest {
	
	private Mesa mesa;
	private CarrocoesPrimeiro estrategia;
	...
	void inicia() throws JogadaInvalidaException {
		...
		this.estrategia = new CarrocoesPrimeiro();
	}
	...
	@Test
	void CarrocoesPrimeiro() throws JogadaInvalidaException {
		Jogada j1 = this.estrategia.decideJogada(mesa, List.of(new Peca(0, 2), new Peca(2, 2)));
		...
		Jogada j2 = this.estrategia.decideJogada(mesa, List.of(new Peca(0, 2), new Peca(3, 2)));	
		assertEquals(TipoJogada.NA_DIREITA, j1.getTipo());
		...
		assertEquals(TipoJogada.NA_DIREITA, j1.getTipo());
		assertEquals(5, j3.getPeca().getNumEsquerdo());
		assertEquals(5, j3.getPeca().getNumDireito());
