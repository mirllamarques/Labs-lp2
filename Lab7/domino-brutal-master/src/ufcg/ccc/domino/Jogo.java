...
public class Jogo {
	 ...
	 */
	public Jogo(String nomeJogador1, EstrategiaDeJogo estrategia1, String nomeJogador2, 
			EstrategiaDeJogo estrategia2, int numPecasInicial) {
	...
	/**
	 * @param vencedor
	 * @return Define que tipo de vitória foi e atribui a pontuação correspondente
	 */
	private String tipoVitoria(Jogador vencedor) {
		String retorno = "";
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (jogadaJ1.getTipo() == TipoJogada.PASSA && jogadaJ2.getTipo() == TipoJogada.PASSA) {
			...
			if(jogador1.getNumPecas() < jogador2.getNumPecas()) {
				...
				this.pontuacao = 1;
				return;
			} else if(jogador1.getNumPecas() > jogador2.getNumPecas()) {
				// J2 venceu
				this.finalizado = true;
				this.vencedor = this.jogador2.getNome();
				this.pontuacao = 1;
...
//		System.out.println("==> final: " + (venceu == null ? "EMPATE" : venceu + " VENCEU") + "\n");
	...
	/**
	 * @return Retorna quantos pontos o vencedor da partida recebeu
	 */
